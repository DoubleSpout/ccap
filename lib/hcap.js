...
var pid = process.pid;
var isLinux = os.platform() === 'linux' || os.platform() === 'darwin';
...
CAP.prototype._init = function(args){
	...
	if(typeof arguments[0] === 'object'){//如果传递了对象，则替换默认值
		...
		var extension = '.bmp';
		if (isLinux) { //判断是否启用jpeg,如果是为win32则只能使用bmp
		  extension = '.' + (obj.type || 'jpeg');
		}
		this.filename = img_path+'/captcha_'+ins_count+'_'+pid+'_'+Date.now()+'_{$number}'+extension;
		return;
	}
	//如果只传递了宽，高，间隔则替换默认值
...
CAP.prototype._call_create = function(text,len,j){//调用C++的CIMG库的create函数生成验证码文件
	...
	hrobj.create(text, filepath, len, that.width, that.height, that.offset, that.quality,isLinux,that.fontsize);
